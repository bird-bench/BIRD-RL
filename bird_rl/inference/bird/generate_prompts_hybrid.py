#!/usr/bin/env python3
"""
Generate turn prompts for BIRD inference using the Hybrid model.

Uses BIRD_SYSTEM_PROMPT + UNIFIED_TOOLS_BLOCK + UNIFIED_TOOL_CALL_INSTRUCTIONS
from bird_rl.prompts.hybrid_prompts.
...
from bird_rl.prompts.hybrid_prompts import (
    BIRD_SYSTEM_PROMPT,
    BIRD_USER_TEMPLATE,
    UNIFIED_TOOLS_BLOCK,
    UNIFIED_TOOL_CALL_INSTRUCTIONS,
)
...
def process_dataset(
    ...
    """
    Process BIRD dev dataset and generate turn prompts using hybrid format.
    ...
    for item in data:
        ...
        # Hybrid format: BIRD_SYSTEM_PROMPT with unified tools
        system_prompt = BIRD_SYSTEM_PROMPT.format(
            tools_block=UNIFIED_TOOLS_BLOCK,
            tool_call_instructions=UNIFIED_TOOL_CALL_INSTRUCTIONS.format(
                max_turns=max_turns,
                prev_turns=max_turns - 1
            )
        )

        user_prompt = BIRD_USER_TEMPLATE.format(
...
def main():
    parser = argparse.ArgumentParser(description='Generate turn prompts for BIRD hybrid inference')
