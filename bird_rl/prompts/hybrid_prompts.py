"""
Unified prompt templates for Hybrid model inference.

The hybrid model uses UNIFIED_TOOLS_BLOCK (with sql_list for submit_solution)
and can handle both BIRD (SQL generation) and Critic (SQL debugging) tasks.

These prompts are used by hybrid models trained on both tasks simultaneously.
"""

UNIFIED_TOOLS_BLOCK = """<tools>
[
  {
    "type": "function",
    "function": {
      "name": "execute_sql",
      "description": "Execute a SQL query against the database for exploration and testing. Returns execution results but does NOT evaluate correctness. Use this to test and refine your SQL before submitting the final solution.",
      "parameters": {
        "type": "object",
        "properties": {
          "sql": {
            "type": "string",
            "description": "A single SQL query to execute for exploration and testing."
          }
        },
        "required": ["sql"]
      }
  ...
  },
  {
    ...
    "function": {
      ...
      "description": "Submit your final SQL solution. This ends the current task. The SQL will be executed and evaluated against test cases. Only call this when you are confident your solution is correct, as this is your final submission.",
      "parameters": {
        "type": "object",
        "properties": {
          "sql_list": {
...
UNIFIED_TOOL_CALL_INSTRUCTIONS = """## How to Call Tools
...
1. **<think> tags**: Concise, high-level reasoning about what you're doing and why.
...
BIRD_SYSTEM_PROMPT = """You are an expert SQL query writer. Your task is to translate natural language questions into correct SQL queries.
...
BIRD_USER_TEMPLATE = """## Database Schema
{schema}
...
**Note**: If conversation history is provided below, continue from where the exploration left off. If this is the first turn, start by analyzing the question and exploring the database."""
