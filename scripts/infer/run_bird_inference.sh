#!/bin/bash
# Multi-turn inference pipeline for the BIRD-specialized model.
# submit_solution uses single `sql` string format.
#
# Usage:
...
echo "============================================================"
echo "BIRD Multi-Turn Inference Pipeline"
echo "============================================================"
...
for TURN in $(seq 0 $((MAX_TURNS - 1))); do
    ...
    python -m bird_rl.inference.bird.generate_prompts \
        --turn "${TURN}" \
        --max-turns "${MAX_TURNS}" \
        --dev-data "${DEV_DATA}" \
        --db-dir "${DB_DIR}" \
