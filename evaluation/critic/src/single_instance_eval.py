...
def run_test_case(
    test_code, result, logger, conn, pred_sqls, sol_sqls, db_path, kwargs
):
...
def execute_test_cases(
    test_cases, sql_result, logger, conn, pred_sqls, sol_sqls, db_path, kwargs
):
...
def run_preprocessing(preprocess_sql, db_path, logger, conn):
    ...
    if preprocess_sql:
        execute_queries(
            preprocess_sql, db_path, conn, logger, section_title="Preprocess SQL"
        )
...
def evaluate_instance(data, args, logger):
    ...
    required_fields = ["preprocess_sql", "sol_sql"]
    if args.mode == "pred":
    ...
    db_name = data.get("selected_database", data.get("db_id", "unknown"))
    ...
    # Load issue_sql for problem demonstration test
    ...
    # Set up kwargs
    # ALWAYS pass conditions to test cases (even custom ones need it)
    ...
    try:
        ...
        logger.info("Initial database reset")
        reset_and_restore_database(db_path, db_name, logger)

        # ==== PHASE 1: Test issue_sql (Problem Demonstration) ====
        # Test issue_sql (expected to FAIL)
        issue_sql_passed_unexpectedly = False
        issue_sql_error_message = ""
        logger.info("=== Phase 1: Testing issue_sql (Problem Demonstration) ===")
        ...
        if issue_sql:
            ...
            # Check if issue_sql PASSED when it should FAIL
            if (
                not issue_exec_error
                and not issue_timeout_error
                and not issue_assertion_error
            ):
                issue_sql_passed_unexpectedly = True
                issue_sql_error_message = "⚠️ UNEXPECTED: issue_sql passed all tests when it should demonstrate a problem!\n"
                logger.warning(issue_sql_error_message)
            else:
                logger.info("✓ issue_sql failed as expected (demonstrates the problem)")

        # Reset database after issue_sql test
        logger.info("Resetting database after issue_sql test")
        reset_and_restore_database(db_path, db_name, logger)

        # Reconnect after reset
        db_connection = get_connection_for_phase(db_path, logger)

        # ==== PHASE 2: Test sol_sql (Solution) ====
        logger.info("=== Phase 2: Testing sol_sql (Solution) ===")

        # Run preprocessing SQL again
        run_preprocessing(preprocess_sql, db_path, logger, db_connection)

        # Test sol_sql (expected to PASS)
        (
        ...
        if issue_sql_passed_unexpectedly:
            eval_error_message = issue_sql_error_message + eval_error_message
        ...
        # Determine status (fail if sol_sql fails OR issue_sql passed unexpectedly)
        ret_status = "success"
        if (
            evaluation_phase_execution_error
            or evaluation_phase_timeout_error
            or evaluation_phase_assertion_error
            or issue_sql_passed_unexpectedly
        ):
            ret_status = "failed"
        ...
        return {
            ...
            "issue_sql_passed_unexpectedly": issue_sql_passed_unexpectedly,
        }
